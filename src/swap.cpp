 ...
 *
 *    $Id: swap.c 5800 2013-08-14 14:24:05Z paulf $
 *
 *    Revision history:
 *     $Log$
...
void SwapLong( long *data )
{
   ...
   if (sizeof(long) != 4) {
      fprintf(stderr, "FATAL ERROR: SwapLong() assumes 4 byte longs and sizeof(long)=%ld\n", (long) sizeof(long));
      fprintf(stderr, "FATAL ERROR: was Earthworm compiled with -m32 flag?\n");
      exit(9);
   }
...
static int WaveMsgVersionMakeLocal( TRACE2X_HEADER* wvmsg, char version )
{
  ...
  if (wvmsg->nsamp > 4032/dataSize) {
     /*qDebug() << "WaveMsg2MakeLocal: packet from"
              << wvmsg->sta << wvmsg->chan
              << wvmsg->net << wvmsg->loc
              << "has bad number of samples="
              << wvmsg->nsamp << "datatype="
              << wvmsg->datatype;*/
     return(-1);
  }
